...
const { TabPane } = Tabs;  //结构语法     // const TabPane = Tabs.TabPane;
...
export class StayDetailInfoButton extends React.Component { //有export 是因为host, guest都要用
  state = {
    modalVisible: false,
  };
...
// JSON javascript object notation  1.subset  2.superclass
class MyStays extends React.Component {
  ...
  loadData = async () => {
    ...
    try {
      const resp = await getStaysByHost(); //任何函数要使用await,都要被标记为async()
      this.setState({
        data: resp,
      });
    } catch (error) {
...
class HostHomePage extends React.Component {  //负责显示tabs, 每个tabpane的内容
  render() {
    return (
